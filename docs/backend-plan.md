...
| 9 Code on the screen | done · 2026-09-06 | six digits from a per-boot secret and the minute, shown only on loopback (`/v1/screen`, a window on the Mac), five misses lock the minute; name + code signs a device in |
| 10 Invitations and guests | done · 2026-09-06 | invitation links that work once for seven days, the person exists as pending until they hold a passkey, guests carry an expiry that ends their sessions; `/join` page; Settings shows real members and pending invites |
| 11 Data rights | done · 2026-09-06 | export (person or household) as a folder under the data root with manifest, JSON and objects; delete account leaves only a ledger row; transfer ownership is the class H action confirmed with a passkey |
...
| 22 Media | done · 2026-09-06 | ffprobe on arrival, a library by kind, byte-range serving so players seek, on-the-fly ffmpeg transcoding to fragmented MP4 where a screen cannot play the original, the TV page as the living-room screen (fill the screen over HDMI, videos, music, photo slideshow) |
...
| 27 Routines | done · 2026-09-06 | routines are lists of ordinary capability calls run with their author's role, so they can do no more than the person who made them; triggers by time (a minute tick), presence (Leaving, Arrive), phrase (Tandem later) or by hand; class D steps wait as approval cards; every new house starts with Goodnight, Leaving and Arrive; a builder on the Home page |
| 36 Memory | done · 2026-09-06 | per-person memory nobody else can read, the owner included; a remark Tandem notices is a candidate that becomes durable only when confirmed or noticed again, otherwise forgotten in seven days; retention per person applied to what already exists; forget blanks the text so the next snapshot carries nothing; receipts never carry the text; export and account deletion include it |
| 44 The screen | done · 2026-09-06 | the loopback screen page shows Ready or Needs a look, who is home, the Gate, cameras, approvals waiting, alerts, recent activity titles and the pairing code; the state endpoint is loopback-only |
