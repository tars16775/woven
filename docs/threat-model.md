...
- **At rest.** The volume is an APFS image the household can encrypt with FileVault; keys live in `keys/` (0700) and the CA private key never leaves the box. Diagnostics bundles and exports pass through the scrub filter (`src/diagnostics.ts`). Full encryption at rest with a household key is a hardening item for the box image (phase 49).
