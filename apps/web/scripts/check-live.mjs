...
function walk(dir) {
  for (const name of readdirSync(dir)) {
    ...
    else if (/^live[^/]*\.tsx?$/.test(name) || /^(pilot-card|memory-card|passkeys-card|backups-card|data-rights-card|backup-tokens-card|search-box|approvals)\.tsx$/.test(name)) check(p);
  }
}
