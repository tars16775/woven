...
beforeAll(async () => {
  ...
  child = cookieOf(await app.inject({ method: "POST", url: "/v1/auth/recover", payload: { email: "sam@example.com", code: services.recovery.issue(sam)[0] } }));
});
afterAll(async () => {
  await app.close();
  data.close();
...
describe("uploads (phase 18)", () => {
  it("uploads in chunks, resumes, verifies the hash, and dedups", async () => {
    ...
    const start = await app.inject({ method: "POST", url: "/v1/files/uploads", headers: { cookie: owner }, payload: { name: "big.bin", path: "/Backups/laptop", size: bytes.length, sha256: sha(bytes), mime: "application/octet-stream" } });
    ...
    const wrongSize = await app.inject({ method: "PUT", url: `/v1/files/uploads/${session.id}/chunks/0`, headers: { cookie: owner, "content-type": "application/octet-stream" }, payload: bytes.subarray(0, 100) });
    expect(wrongSize.statusCode).toBe(400);
    const c1 = await app.inject({ method: "PUT", url: `/v1/files/uploads/${session.id}/chunks/1`, headers: { cookie: owner, "content-type": "application/octet-stream" }, payload: bytes.subarray(CHUNK_SIZE) });
    expect(c1.json<{ received: number[] }>().received).toEqual([1]);
    const early = await app.inject({ method: "POST", url: `/v1/files/uploads/${session.id}/complete`, headers: { cookie: owner } });
    expect(early.statusCode).toBe(409);
    // A "new device" asks where it got to, then sends what is missing.
    const status = await app.inject({ method: "GET", url: `/v1/files/uploads/${session.id}`, headers: { cookie: owner } });
    expect(status.json<{ received: number[] }>().received).toEqual([1]);
    await app.inject({ method: "PUT", url: `/v1/files/uploads/${session.id}/chunks/0`, headers: { cookie: owner, "content-type": "application/octet-stream" }, payload: bytes.subarray(0, CHUNK_SIZE) });
    const done = await app.inject({ method: "POST", url: `/v1/files/uploads/${session.id}/complete`, headers: { cookie: owner } });
    ...
    const again = await app.inject({ method: "POST", url: "/v1/files/uploads", headers: { cookie: owner }, payload: { name: "copy.bin", path: "/Backups/desktop", size: bytes.length, sha256: sha(bytes) } });
    expect(again.json<{ alreadyStored: boolean }>().alreadyStored).toBe(true);
    const copy = await app.inject({ method: "POST", url: `/v1/files/uploads/${again.json<{ id: string }>().id}/complete`, headers: { cookie: owner } });
    expect(copy.statusCode).toBe(201);
    expect(copy.json<Entry>().sha256).toBe(entry.sha256);
    const summary = await app.inject({ method: "GET", url: "/v1/files/summary", headers: { cookie: owner } });
    ...
    const lied = await app.inject({ method: "POST", url: "/v1/files/uploads", headers: { cookie: owner }, payload: { name: "liar.bin", path: "/", size: 5, sha256: "a".repeat(64) } });
    await app.inject({ method: "PUT", url: `/v1/files/uploads/${lied.json<{ id: string }>().id}/chunks/0`, headers: { cookie: owner, "content-type": "application/octet-stream" }, payload: Buffer.from("hello") });
    const bad = await app.inject({ method: "POST", url: `/v1/files/uploads/${lied.json<{ id: string }>().id}/complete`, headers: { cookie: owner } });
...
describe("files (phase 19)", () => {
  it("lists folders derived from paths, downloads, moves, shares, and keeps namespaces apart", async () => {
    ...
    const root = await app.inject({ method: "GET", url: "/v1/files?namespace=personal&path=/", headers: { cookie: owner } });
    ...
    const docs = await app.inject({ method: "GET", url: "/v1/files?namespace=personal&path=/Documents", headers: { cookie: owner } });
    ...
    const lease = docs.json<Listing>().folders.length ? (await app.inject({ method: "GET", url: "/v1/files?namespace=personal&path=/Documents/Home", headers: { cookie: owner } })).json<Listing>().files[0]! : null;
    expect((await app.inject({ method: "GET", url: `/v1/files/${lease!.id}/content`, headers: { cookie: child } })).statusCode).toBe(403);
    const dl = await app.inject({ method: "GET", url: `/v1/files/${lease!.id}/content?download=true`, headers: { cookie: owner } });
    ...
    const samShared = await app.inject({ method: "GET", url: "/v1/files?namespace=household", headers: { cookie: child } });
    expect(samShared.json<Listing>().files.map((f) => f.name)).toEqual(["wifi.txt"]);
    expect((await app.inject({ method: "GET", url: "/v1/files?namespace=security", headers: { cookie: child } })).statusCode).toBe(403);

    // Sharing = moving into the household namespace; it leaves a receipt. A child cannot move Alex's file.
    expect((await app.inject({ method: "POST", url: `/v1/files/${lease!.id}/move`, headers: { cookie: child }, payload: { namespace: "household" } })).statusCode).toBe(403);
    const moved = await app.inject({ method: "POST", url: `/v1/files/${lease!.id}/move`, headers: { cookie: owner }, payload: { namespace: "household", path: "/Home", name: "lease-2026.pdf" } });
    ...
    expect((await app.inject({ method: "GET", url: `/v1/files/${lease!.id}/content`, headers: { cookie: child } })).statusCode).toBe(200);
    ...
    expect((await app.inject({ method: "DELETE", url: `/v1/files/${shared.id}`, headers: { cookie: owner } })).statusCode).toBe(200);
    expect(await data.store.has(shared.sha256)).toBe(true);
    await app.inject({ method: "DELETE", url: `/v1/files/${dup.id}`, headers: { cookie: owner } });
  ...
  it("the backup walk hashes files and skips sidecars and hidden folders", async () => {
    ...
    const listing = (await app.inject({ method: "GET", url: "/v1/files?namespace=personal&path=/Backups/mac/walk", headers: { cookie: owner } })).json<Listing>();
