...
describe("routines (phase 27)", () => {
  it("a new house starts with three routines", async () => {
    const res = await app.inject({ method: "GET", url: "/v1/routines", headers: { cookie: owner } });
  ...
  it("running Goodnight turns things off, sets the thermostat, locks the door, and leaves receipts", async () => {
    ...
    const list = (await app.inject({ method: "GET", url: "/v1/routines", headers: { cookie: owner } })).json<{ routines: R[] }>().routines;
    const goodnight = list.find((r) => r.name === "Goodnight")!;
    const run = await app.inject({ method: "POST", url: `/v1/routines/${goodnight.id}/run`, headers: { cookie: owner } });
    ...
    const after = (await app.inject({ method: "GET", url: "/v1/routines", headers: { cookie: owner } })).json<{ routines: R[] }>().routines.find((r) => r.name === "Goodnight")!;
    expect(after.lastResult).toBe("4 of 4 steps done");
  });

  it("a phrase runs the matching routine", async () => {
    const res = await app.inject({ method: "POST", url: "/v1/routines/say", headers: { cookie: owner }, payload: { phrase: "Goodnight" } });
    expect(res.json<Run>().summary).toMatch(/4 of 4/);
    expect((await app.inject({ method: "POST", url: "/v1/routines/say", headers: { cookie: owner }, payload: { phrase: "party time" } })).json()).toBeNull();
  });
  ...
  it("a routine can do no more than its author; a class D step still asks", async () => {
    ...
    const made = await app.inject({ method: "POST", url: "/v1/routines", headers: { cookie: child }, payload: { name: "Sneak", trigger: { kind: "manual" }, steps: [{ capability: "lock.unlock", target: "entry.front-door", parameters: {} }, { capability: "light.set", target: "bed.bedside", parameters: { on: true } }] } });
    expect(made.statusCode).toBe(201);
    const run = await app.inject({ method: "POST", url: `/v1/routines/${made.json<R>().id}/run`, headers: { cookie: child } });
    ...
    const ownerMade = await app.inject({ method: "POST", url: "/v1/routines", headers: { cookie: owner }, payload: { name: "Open up", trigger: { kind: "manual" }, steps: [{ capability: "lock.unlock", target: "entry.front-door", parameters: {} }] } });
    const run2 = (await app.inject({ method: "POST", url: `/v1/routines/${ownerMade.json<R>().id}/run`, headers: { cookie: owner } })).json<Run>();
  ...
  it("presence flipping runs Leaving and Arrive; the schedule runs on the minute", async () => {
    ...
    await app.inject({ method: "POST", url: "/v1/routines", headers: { cookie: owner }, payload: { name: "Kettle", trigger: { kind: "time", at: hhmm }, steps: [{ capability: "plug.set", target: "kitchen.kettle", parameters: { on: true } }] } });
  ...
  it("only the owner or the author changes or deletes a routine", async () => {
    ...
    expect((await app.inject({ method: "PATCH", url: `/v1/routines/${list[0]!.id}`, headers: { cookie: child }, payload: { enabled: false } })).statusCode).toBe(400);
    expect((await app.inject({ method: "PATCH", url: `/v1/routines/${sneak.id}`, headers: { cookie: child }, payload: { enabled: false } })).json<R>().enabled).toBe(false);
    expect((await app.inject({ method: "DELETE", url: `/v1/routines/${sneak.id}`, headers: { cookie: owner } })).json()).toEqual({ removed: true });
  });
});
