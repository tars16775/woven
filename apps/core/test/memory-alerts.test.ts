...
beforeAll(async () => {
  ...
  maya = sessionOf(await app.inject({ method: "POST", url: "/v1/auth/recover", payload: { email: "maya@example.com", code: services.recovery.issue(m)[0] } }));
});
afterAll(async () => {
  await app.close();
  data.close();
...
describe("memory (phase 36)", () => {
  it("what a person says is kept; what Tandem overhears is a candidate until it comes up again", async () => {
    const said = await app.inject({ method: "POST", url: "/v1/memory", headers: auth(owner), payload: { text: "I take my coffee black", kind: "preference" } });
    ...
    const durableOnly = await app.inject({ method: "GET", url: "/v1/memory?candidates=false", headers: auth(owner) });
  ...
  it("is private even from the owner, and edit, confirm and forget are the person's own", async () => {
    const mine = await app.inject({ method: "POST", url: "/v1/memory", headers: auth(maya), payload: { text: "My sister's birthday is in May" } });
    const id = mine.json<M>().id;
    const ownerList = await app.inject({ method: "GET", url: "/v1/memory", headers: auth(owner) });
    expect(JSON.stringify(ownerList.json())).not.toContain("sister");
    expect((await app.inject({ method: "PATCH", url: `/v1/memory/${id}`, headers: auth(owner), payload: { text: "x" } })).statusCode).toBe(404);
    expect((await app.inject({ method: "PATCH", url: `/v1/memory/${id}`, headers: auth(maya), payload: { text: "My sister's birthday is 12 May" } })).json<M>().text).toBe("My sister's birthday is 12 May");
    expect((await app.inject({ method: "DELETE", url: `/v1/memory/${id}`, headers: auth(maya) })).statusCode).toBe(200);
    expect((await app.inject({ method: "GET", url: "/v1/memory", headers: auth(maya) })).json<{ memories: M[] }>().memories).toHaveLength(0);
...
describe("alerts and metrics (phases 44 and 47)", () => {
  ...
  it("metrics count by route pattern and status class, never by path", async () => {
    await app.inject({ method: "GET", url: "/v1/memory", headers: auth(owner) });
    const res = await app.inject({ method: "GET", url: "/v1/system/metrics", headers: auth(owner) });
    expect(res.statusCode).toBe(200);
    ...
    expect((await app.inject({ method: "GET", url: "/v1/system/alerts", headers: auth(maya) })).json()).toEqual({ alerts: [] });
  });
});
