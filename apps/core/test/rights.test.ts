...
describe("the code on the screen (phase 9)", () => {
  ...
  it("signs a device in by name and code, for people who exist", async () => {
    ...
    expect(cookieOf(ok)).toMatch(/^woven_session=/);
  });
});
...
describe("invitations (phase 10)", () => {
  ...
  it("an adult is invited with a link that works once", async () => {
    const res = await app.inject({ method: "POST", url: "/v1/household/invitations", headers: { cookie: owner }, payload: { name: "Maya", email: "maya@example.com", role: "adult" } });
    expect(res.statusCode).toBe(201);
    const inv = res.json<{ token: string; person: { role: string; name: string } }>();
    expect(inv.person).toMatchObject({ name: "Maya", role: "adult" });
    token = inv.token;
    const pending = await app.inject({ method: "GET", url: "/v1/household/invitations", headers: { cookie: owner } });
    ...
    expect((await app.inject({ method: "GET", url: "/v1/household/invitations", headers: { cookie: owner } })).json<{ invitations: unknown[] }>().invitations).toHaveLength(0);
  });

  it("a guest needs a stay length and stops working when it ends", async () => {
    const noDays = await app.inject({ method: "POST", url: "/v1/household/invitations", headers: { cookie: owner }, payload: { name: "Pat", role: "guest" } });
    expect(noDays.statusCode).toBe(400);
    const res = await app.inject({ method: "POST", url: "/v1/household/invitations", headers: { cookie: owner }, payload: { name: "Pat", role: "guest", guestDays: 3 } });
    ...
    const cookie = cookieOf(await app.inject({ method: "POST", url: "/v1/auth/recover", payload: { email: "pat@example.com", code } }).then((r) => (r.statusCode === 200 ? r : { headers: {} as Record<string, unknown> })));
    // Guests without an email sign in by the screen code.
    const { code: screen } = services.screen.current();
    const session = cookieOf(await app.inject({ method: "POST", url: "/v1/auth/code/login", payload: { name: "Pat", code: screen } }));
    expect(session || cookie).toMatch(/woven_session=/);
    expect((await app.inject({ method: "GET", url: "/v1/auth/session", headers: { cookie: session } })).statusCode).toBe(200);
    data.database.db.update(people).set({ expiresAt: new Date(Date.now() - 1000).toISOString() }).where(eq(people.id, guestId)).run();
    expect((await app.inject({ method: "GET", url: "/v1/auth/session", headers: { cookie: session } })).statusCode).toBe(401);
  });

  it("withdrawing an invitation removes the pending person", async () => {
    const res = await app.inject({ method: "POST", url: "/v1/household/invitations", headers: { cookie: owner }, payload: { name: "Temp", role: "child" } });
    const id = res.json<{ id: string }>().id;
    expect((await app.inject({ method: "DELETE", url: `/v1/household/invitations/${id}`, headers: { cookie: owner } })).json()).toEqual({ withdrawn: true });
...
describe("data rights (phase 11)", () => {
  it("exports a person's data as a folder with a manifest and their objects", async () => {
    ...
    const res = await app.inject({ method: "POST", url: "/v1/household/export", headers: { cookie: owner }, payload: { scope: "me" } });
  ...
  it("deletes an account: no sessions, no keys, no files, no objects, and a ledger row that it happened", async () => {
    ...
    const cookie = cookieOf(await app.inject({ method: "POST", url: "/v1/auth/recover", payload: { email: "maya@example.com", code } }));
    ...
    const asSelfOnOwner = await app.inject({ method: "DELETE", url: `/v1/household/people/${ownerId}/account`, headers: { cookie } });
    expect(asSelfOnOwner.statusCode).toBe(400);
    const res = await app.inject({ method: "DELETE", url: `/v1/household/people/${maya.id}/account`, headers: { cookie } });
    expect(res.statusCode).toBe(200);
  ...
  it("transfers ownership only through the class H action with a passkey, and the roles swap", async () => {
    const res = await app.inject({ method: "POST", url: "/v1/household/people", headers: { cookie: owner }, payload: { name: "Jo", email: "jo@example.com", role: "adult" } });
    const jo = res.json<{ id: string }>().id;
    const prepared = await app.inject({ method: "POST", url: "/v1/actions/prepare", headers: { cookie: owner }, payload: { capability: "household.transfer_ownership", target: "household", parameters: { toPersonId: jo } } });
    expect(prepared.json<{ status: string; approval: { factors: string[] } }>()).toMatchObject({ status: "prepared", approval: { factors: ["strong_auth"] } });
    const noKey = await app.inject({ method: "POST", url: `/v1/actions/${prepared.json<{ id: string }>().id}/execute`, headers: { cookie: owner } });
